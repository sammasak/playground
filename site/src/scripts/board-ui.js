...
function makeMove(uci) {
  try {
    ...
    // Personality chat: if this move captured a piece, fire a quip.
    // Fire-and-forget — never blocks the move/animation, never throws.
    if (toPiece && announceCapture) {
      const moverIsBot = moverColor === 'white'
        ? !!state.bots.white
        : !!state.bots.black;
      announceCapture({
        type: 'capture',
