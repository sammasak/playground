...
export function init(deps) {
  ...
  announceCapture = deps.announceCapture;
  renderBoard = deps.renderBoard;
  updateUI = deps.updateUI;
  switchToTab = deps.switchToTab;
}

// Detect a capture for a bot move and fire a quip.
// Must be called BEFORE state.game.makeMove(uci): reads the destination piece
// (and mover) from the pre-move board. Never throws.
function reportBotCapture(uci) {
  try {
    if (!announceCapture || !state.game) return;
    const boardState = state.game.getBoardState();
    const fileToIdx = (sq) => (parseInt(sq[1]) - 1) * 8 + (sq.charCodeAt(0) - 97);
    const fromPiece = boardState.squares[fileToIdx(uci.slice(0, 2))];
    const toPiece = boardState.squares[fileToIdx(uci.slice(2, 4))];
    if (!toPiece || !fromPiece) return; // not a capture

    const moverColor = state.game.getTurn(); // side to move BEFORE the move
    const moverIsBot = moverColor === 'white' ? !!state.bots.white : !!state.bots.black;
    announceCapture({
      type: 'capture',
      attacker: { color: moverColor, pieceType: fromPiece.pieceType },
      victim: { color: toPiece.color, pieceType: toPiece.pieceType },
      byBot: moverIsBot,
    });
  } catch (e) {
    // Never let a quip break a bot move.
    console.warn('reportBotCapture failed:', e);
  }
}
