...
test.describe('Chess Game', () => {
  test.beforeEach(async ({ page }) => {
    // Use mock LLM mode: these tests exercise the chess game, not the bot, so
    // there's no reason to spin up the ~880MB on-device model worker (which only
    // adds load/races to the chess-engine wasm render).
  ...
  test('should reset the game when clicking New Game', async ({ page }) => {
    ...
    // Confirm the in-DOM confirmation dialog (a custom overlay, not a native dialog)
    ...
    // Move history should be cleared (the reset move e2e4 is gone; an
    // empty-state placeholder may render in its place).
    const movesList = page.locator('#moves-list');
    await expect(movesList).not.toContainText('e2e4');
  });
